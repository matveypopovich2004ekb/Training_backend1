...
from app.services.categories import CategoryNotFountError, CategoryService
...
def create_new_category(
    new_ctgr: CategoryCreate, service: CategoryService = Depends(get_category_service)
) -> CategoryRead:
...
@router.delete(
    "/{category_id}", response_model=None, status_code=status.HTTP_204_NO_CONTENT
)
def delete_category(
    category_id, service: CategoryService = Depends(get_category_service)
) -> None:
    try:
