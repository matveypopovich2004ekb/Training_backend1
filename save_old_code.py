...
SessionLocal = sessionmaker(
    bind=engine
)  # создание штуки, которая может открывать сессию
...
app = FastAPI(
    lifespan=lifespan
)  # lifespan - фуе-яЮ которая выполняется до хзапусска приложеня
...
def delete_task(task_id: str, db: Session = Depends(get_db)) -> None:
    ...
    if task is None:
        raise HTTPException(
            ...
            detail="Брат задача не найдена брат:( брат",
        )
    db.delete(task)
    db.commit()
...
def new_category(
    user_input_category: Create_Category, db: Session = Depends(get_db)
) -> Category:
...
def get_category_list(db: Session = Depends(get_db)) -> list[Category]:
    category_list = db.scalars(
        select(CategoryORM)
    ).all()  # all() сам переделывает ответ в список объектов CategoryORM
...
def del_category(cat_id: str, db: Session = Depends(get_db)) -> None:
    categ = db.get(
        CategoryORM, cat_id
    )  # если строка с cat_id найдеься, то она попадет в эту переменную, инача в переменную попоадет None
    if categ is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def upd_category(
    category_id: str,
    new_text_of_category: Update_Category,
    ...
    categ.name = (
        new_text_of_category.name
        if new_text_of_category.name is not None
        else categ.name
    )
    db.commit()
